...
router.get('/', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log('GET /products - Response sent');
    } catch (error) {
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log(`GET /products/${req.params.id} - Response sent`);
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
        console.error(`GET /products/${req.params.id} - Error:`, error.message);
    }
}));
        ...
        uploadProduct.fields([
            ...
            fields.forEach((field, index) => {
                if (req.files[field] && req.files[field].length > 0) {
                    ...
                    const imageUrl = `http://SERVER_IP:3000/image/products/${file.filename}`;
