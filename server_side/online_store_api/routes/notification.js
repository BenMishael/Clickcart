...
router.post('/send-notification', asyncHandler(async (req, res) => {
    console.log('POST /notification/send-notification - Request received');
    ...
    console.log('POST /notification/send-notification - Response sent');
}));

router.get('/track-notification/:id', asyncHandler(async (req, res) => {
    console.log(`GET /notification/track-notification/${req.params.id} - Request received`);
    ...
    console.log(`GET /notification/track-notification/${req.params.id} - Response sent`);
}));


router.get('/all-notification', asyncHandler(async (req, res) => {
    console.log('GET /notification/all-notification - Request received');
    try {
        const notifications = await Notification.find({}).sort({ _id: -1 });
        res.json({ success: true, message: "Notifications retrieved successfully.", data: notifications });
        console.log('GET /notification/all-notification - Response sent');
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
        console.error('GET /notification/all-notification - Error:', error.message);
    }
}));


router.delete('/delete-notification/:id', asyncHandler(async (req, res) => {
    console.log(`DELETE /notification/delete-notification/${req.params.id} - Request received`);
    const notificationID = req.params.id;
    try {
        ...
        console.log(`DELETE /notification/delete-notification/${req.params.id} - Response sent`);
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
        console.error(`DELETE /notification/delete-notification/${req.params.id} - Error:`, error.message);
    }
}));
