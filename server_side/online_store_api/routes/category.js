...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log(`GET /categories/${req.params.id} - Response sent`);
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
        console.error(`GET /categories/${req.params.id} - Error:`, error.message);
    }
}));
...
router.post('/', asyncHandler(async (req, res) => {
    try {
        uploadCategory.single('img')(req, res, async function (err) {
            ...
            let imageUrl = 'no_url';
            if (req.file) {
                imageUrl = `http://SERVER_IP:3000/image/category/${req.file.filename}`;
            }
...
router.put('/:id', asyncHandler(async (req, res) => {
    try {
        ...
        uploadCategory.single('img')(req, res, async function (err) {
            ...
            if (req.file) {
                image = `http://SERVER_IP:3000/image/category/${req.file.filename}`;
            }
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log(`DELETE /categories/${req.params.id} - Response sent`);
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
        console.error(`DELETE /categories/${req.params.id} - Error:`, error.message);
    }
}));
